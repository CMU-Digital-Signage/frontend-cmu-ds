...
const addSchedule = () => {
  ...
  if (
    ...
    (selectedContentType.value.code !== CONTENT_CODE.Video &&
...
const nextStepUpload = () => {
  if (
    ...
    if (!formPoster.value.image) {
      ...
      if (selectedContentType.value.code === CONTENT_CODE.Webview) {
    ...
    maxContent.value = Math.min(...numImage);
    switch (selectedContentType.value.code) {
      case CONTENT_CODE.Video:
        if (maxContent.value > 300) {
          maxContent.value = 300;
        }
        break;
      case CONTENT_CODE.Poster:
        if (maxContent.value > 50) {
          maxContent.value = 50;
        }
        break;
    }
      ...
      <div v-if="currentState === 1">
        <div
          v-if="
            ...
            "
        >
          <div class="bg-[#f2f2f2] p-2 px-4 rounded-lg justify-center my-3">
            <div class="text-[14px] text-center text-red-500">
              Upload limit: {{ maxContent }}
              {{
                selectedContentType.code === CONTENT_CODE.Poster
                  ? `contents`
                  : `seconds`
              }}
            </div>
            <div
              ...
              class="text-[14px] text-center text-[#41b8a2]"
            >
              Uploaded content:
              {{
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    >
                      <div
                        v-if="selectedContentType.code === CONTENT_CODE.Webview"
                      ...
                      >
                        ...
                        <iframe
                          ...
                          :src="`${(formPoster.image[0].image as string).endsWith('.pdf')? `${formPoster.image[0].image}#toolbar=0&navpanes=0`: formPoster.image[0].image }`"
                          :width="`${2160 / 2}px`"
                          :height="`${3840 / 2}px`"
                          scrolling="no"
                          fullScreen="true"
                          ...
                          style="transform: scale(0.167); transform-origin: 0 0"
                        />
                      </div>
                      <video
                        v-else-if="
                          selectedContentType.code === CONTENT_CODE.Video
                        "
                        muted
                        autoplay
                        loop
